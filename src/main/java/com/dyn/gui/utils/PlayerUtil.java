...
public abstract class PlayerUtil {
	...
	public static void applyPotionEffects(EntityPlayer player, String effectString) {
		String[] effects = effectString.replaceAll("\\s", "").split(","); // example
																			// =
																			// 9:5:0
		for (String poisonEffect : effects) {
			String[] effectValues = poisonEffect.split(":");
			if (effectValues.length < 2) {
				// LoggingHandler.felog.warn("Too few arguments for potion
				// effects");
				...
				try {
					...
					player.addPotionEffect(
							new net.minecraft.potion.PotionEffect(potionID, effectDuration * 20, amplifier));
	...
	public static EntityPlayerMP getPlayerByUuid(UUID uuid) {
		MinecraftServer mc = MinecraftServer.getServer();
		List<EntityPlayerMP> playerList = (mc.getConfigurationManager() == null) ? new ArrayList<>()
	...
	public static MovingObjectPosition getPlayerLookingSpot(EntityPlayer player) {
		if (player instanceof EntityPlayerMP) {
			return PlayerUtil.getPlayerLookingSpot(player,
					((EntityPlayerMP) player).theItemInWorldManager.getBlockReachDistance());
		} else {
			return PlayerUtil.getPlayerLookingSpot(player, 5);
		}
	}
	...
	public static MovingObjectPosition getPlayerLookingSpot(EntityPlayer player, double maxDistance) {
		...
		Vec3 playerPos = new Vec3(player.posX, player.posY + (player.getEyeHeight() - player.getDefaultEyeHeight()),
				player.posZ);
		Vec3 pos1 = playerPos.addVector(0, player.getEyeHeight(), 0);
		Vec3 pos2 = pos1.addVector(lookAt.xCoord * maxDistance, lookAt.yCoord * maxDistance,
				lookAt.zCoord * maxDistance);
